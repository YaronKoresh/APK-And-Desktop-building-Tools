...
export function Giselle(allCode) {
  ...
  var regOr = '|';
	...
	var regCommands = '' + regOpen + regAnySpace + regOpen + regOpen + 'say' + regCloseOne + regOr + regOpen + 'sql' + regCloseOne + regOr + regOpen + 'break' + regCloseOne + regOr + regOpen + 'closed' + regCloseOne + regOr + regOpen + regLoop + regCloseOne + regCloseOne + regAnySpace + regCloseOne;
		...
		for (i = 0; i < lines.length; i++) {
			...
			if (reg.test(lines[i])) {
				...
				for (j = 0; j < words.length - 1; j++) {
					...
					word = words[j];
					theDev = true;
					(word == 'loop') && (GiselleCalc(Dependent('nextWord==\'/([0-9]){1,}/\' && words[j+2]=="times"', Dependent('parametersDegree==0', Looper(nextWord), '*could not create a loop as a parameter. Please create the loop in a seperate line of code'), Dependent('parametersDegree==0', Looper(true), '*could not create a loop as a parameter. Please create the loop in a seperate line of code')), '"loop"', 2, 0)) && (found = true);
					...
					(found == false) && (word == 'number') && (GiselleCalc(Dependent('lastWord=="as"', '].forEach(_$D$A$T$A$_ => parseInt(_$D$A$T$A$_))', '*unknown command "number"'), '"number"', 2, 0, '-+')) && (found = true);
					...
					(found == false) && (word == /^([a-zA-Z_]){1}([a-zA-Z0-9_]){,}$/) && (GiselleCalc(Dependent('nextWord=="will"', 'var ' + word, Dependent('lastWord=="loop"', word, '*unknown command "' + word + '"')), '"' + word + '"', 2, 0, Dependent('lastWord=="by"', '-+', '++'))) && (found = true);
...
const Home = () => {
	return (
        ...
        <View style={c.X90}>
          ...
          <View style={c.Y90}>
            <View style={c.cell}>
              <Button raised title="code" onPress={Giselle(`
